...
const Chat = () =>{
  const {navigate, messages, useOnLoad, roomName, setMessage, isAuthorized, useSocketListen, isLoading, useJoinRoom} = useController();
          ...
          <div className="bg-cont h-[90%] w-[90%] md:w-[70%] md:h-[70%]">
            <div className="p-[3px] w-full h-full min-h-[510px] bg-[#242424] rounded-lg flex flex-col justify-between">
              ...
              <div className="p-2 h-[8%] w-[100%] flex border-4 border-sky-400 rounded-lg md:mt-3 md:h-[15%] lg:h-[10%]">
                <input onChange={(e) => {setMessage({message: e.target.value})}} className="h-full w-5/6 bg-transparent text-white focus:outline-none focus:ring-0 focus:border-transparent" type="text" name="msg_cont" placeholder="message.."/>
