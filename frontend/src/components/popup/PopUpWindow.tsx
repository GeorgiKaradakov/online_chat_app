...
const PopUpWindow:React.FC<PPType> = ({isOpen, id, btn_id1, btn_id2, popUpName, inputName1, inputName2, inputPlaceHolder1, inputPlaceHolder2, btnText, className, closePopUp, submitPopUp}) => {
  return (
    <div className={`${isOpen ? className : "hidden"} absolute w-[90%] h-1/2 flex flex-col justify-center items-center md:w-2/3 lg:w-1/3`}>
      <div id={id} className={`z-40 w-full h-full bg-gray-950 flex flex-col items-center rounded-xl`}>
        ...
        <div className="w-1/2 h-1/6 flex justify-between items-center">
          <RadioButton id={btn_id1} name="gender" text="male" className="w-1/2 mr-4"/>
          <RadioButton id={btn_id2} name="gender" text="female" className="w-1/2 ml-4"/>
        </div>
