...
def create_chat_room():
    ...
    db.session.add(
        Message(
            content=f"Your room code is '{new_room.room_code}' !",
            user_id=None,
            room_id=new_room.id,
        )
    )
...
def join_chat_room():
    ...
    if "user_id" in session:
        ...
        new_user = User(
            username=username, gender=gender, room_id=room.id, is_connected=False
        )
        db.session.add(new_user)
        db.session.commit()
        session["user_id"] = new_user.id
    ...
    return (
        jsonify(
            {"Message": "User successfully added to the room!", "room_id": room.id}
        ),
        200,
    )


@app.route("/get_room_name", methods=["GET"])
def is_authorized():
    return jsonify({"roomName": get_room_name(session["user_id"] if session else "")})
...
def user_join():
    ...
    if not user.is_connected:
        ...
        new_message = Message(
            content=f"{user.username} has joined the chat room! :)",
            user_id=None,
            room_id=room.id,
        )
